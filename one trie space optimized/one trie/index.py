# index.py
class Bucket():
    def __init__(self, year, disklocstart=-1, disklocend=-1):
        self.year = year
        self.disklocstart = disklocstart
        self.disklocend = disklocend
        # Counter for number of words that end at this node for the specific year
        self.cnt = 0
...
class Node:
    ...
    def set_year_wise_locs(self, year, diskloc):
        """
        Insert or update a bucket for a specific year in a sorted manner.
        """
        left, right = 0, len(self.buckets) - 1
        while left <= right:
            mid = (left + right) // 2
            if self.buckets[mid].year == year:
                # Update existing bucket
                self.buckets[mid].disklocend = diskloc
                self.buckets[mid].cnt += 1
                return
            elif self.buckets[mid].year < year:
                left = mid + 1
            else:
                right = mid - 1
        # Year not found, insert new bucket maintaining sorted order
        new_bucket = Bucket(year=year, disklocstart=diskloc, disklocend=diskloc)
        new_bucket.cnt = 1
        self.buckets.insert(left, new_bucket)
...
class Trie:
    ...
    def traverse(self, word):
        ...
        # print(node.disklocstart, node.cnt)
        return node
    ...
    def set_year_wise_locs(self, name, year, diskloc):
        """
        Set year-wise disk locations for the given name and year.
        """
        node = self.root
        for ch in name:
            if not node.contains_key(ch):
                node.put(ch, Node())
            node.set_year_wise_locs(year, diskloc)
            node = node.get(ch)
        # Last node
        node.set_year_wise_locs(year, diskloc)
        # node.increase_end()
