...
def my_execute(query, idx_stat):
    ...
    # Handle conjunctive queries (both name and year predicates)
        ...
        if yop == "=":
            ...
            valid_years = list(range(idx_stat['min_year'], val + 1))
        elif yop == ">=":
            valid_years = list(range(val, idx_stat['max_year'] + 1))
        ...
        if op == '=' or (op == 'LIKE' and not value.endswith('%')):
            ...
            if node:
                ...
                sorted_buckets = node.buckets  # Already sorted by year
                sorted_years = sorted(valid_years)  # Ensure valid_years is sorted

                bucket_ptr = 0
                year_ptr = 0
                while bucket_ptr < len(sorted_buckets) and year_ptr < len(sorted_years):
                    bucket_year = sorted_buckets[bucket_ptr].year
                    valid_year = sorted_years[year_ptr]
                    # print(bucket_year, valid_year)
                    if bucket_year == valid_year:
                        bucket = sorted_buckets[bucket_ptr]
                        start = bucket.disklocstart
                        end = start + bucket.cnt - 1
                        # print(bucket_year, range(start, end + 1))
                        matching_disk_indices.extend(range(start, end + 1))
                        # matching_disk_indices.extend(range(bucket.disklocstart, bucket.disklocend + 1))
                        bucket_ptr += 1
                        year_ptr += 1
                    elif bucket_year < valid_year:
                        bucket_ptr += 1
                    else:
                        year_ptr += 1
